...
if [[ "${PSPTOOLCHAIN_ALLEGREX_NEWLIB_NOABICALLS:-}" = "1" ]]; then
  export CFLAGS="${CFLAGS:-} ${PSPTOOLCHAIN_ALLEGREX_NOABICALLS_FLAGS}"
  export CXXFLAGS="${CXXFLAGS:-} ${PSPTOOLCHAIN_ALLEGREX_NOABICALLS_FLAGS}"
  ...
  export ASFLAGS="${ASFLAGS:-} ${PSPTOOLCHAIN_ALLEGREX_NOABICALLS_FLAGS}"
fi
